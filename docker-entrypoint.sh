...
# If the user has run the default command and the socket doesn't exist and it is not an ECS ALB Proxy only, fail
if [ "x${ECS_ALB_PROXY_ONLY}" = "x" -a "$socketMissing" = 1 -a "$1" = forego -a "$2" = start -a "$3" = '-r' ]; then
	exit 1
fi
